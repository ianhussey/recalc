...
#' @param a_digits,b_digits,ab_digits Integer.
...
#'
#' Bounds on the recomputed p-value are obtained by corner enumeration of
...
#' @param a_digits,p_a_digits,b_digits,p_b_digits,p_digits Integer.
...
#' @return Tibble, one row per reference distribution.
#' @examples
#' # Rutherford et al. 2017, HAM-D outcome
...
recalc_mediation_p <- function(a, p_a, b, p_b, p = NULL,
  ...
  rows <- list(one_row(NULL))
  for (d in df_t) rows[[length(rows) + 1L]] <- one_row(d)
...
#' distributions). See those functions for the underlying identities and
#' caveats.
#'
#' @param a,b Numeric. Path coefficients.
#' @param p_a,p_b Numeric. Reported per-path p-values.
...
#' @param method One of \code{"sobel"}, \code{"aroian"}, \code{"goodman"}.
#' @param label Optional string included as a \code{label} column.
#' @return Tibble: one M1 row plus one M2 row per reference distribution.
#' @examples
#' # Rutherford et al. 2017, HAM-D outcome
