...
recalc_f_from_r2 <- function(r2, n, k, f = NULL,
  ...
  recomp <- propagate_intervals(
    ...
    inputs = list(r2 = interval_from_digits(r2, r2_digits, rounding = rounding),
  ...
  recalc_result("B1: F = (R^2/k) / ((1-R^2)/(N-k-1))", f,
                reported_interval(f, f_digits, rounding = rounding), recomp)
}
...
recalc_adj_r2 <- function(r2, n, k, adj_r2 = NULL,
  ...
  recalc_result("B3: R^2_adj = 1 - (1-R^2)(N-1)/(N-k-1)", adj_r2,
                reported_interval(adj_r2, adj_r2_digits, rounding = rounding), recomp)
}
...
recalc_r2_from_blocks <- function(delta_r2, r2 = NULL,
  ...
  k <- length(delta_r2)
  inputs <- setNames(lapply(delta_r2, interval_from_digits, delta_r2_digits, rounding = rounding),
                     paste0("d", seq_len(k)))
