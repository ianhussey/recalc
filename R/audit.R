...
audit_regression <- function(
  ...
  for (i in seq_len(k_pred)) {
    ...
    pi_ <- vget(p, i)
    betai <- vget(beta, i)
    cili <- vget(ci_lower, i)
    ciui <- vget(ci_upper, i)
    ryi <- vget(r_y, i)
    sxi <- vget(sd_x, i)
    sri <- vget(sr2, i)
    ...
    if (has(bi) && has(sxi) && has(sd_y)) {
      add(
        recalc_beta_from_b(
          ...
          beta = betai,
          b_digits = b_digits,
          sd_x_digits = sd_x_digits,
          sd_y_digits = sd_y_digits,
  ...
  if (has(f) && has(n) && has(k)) {
    add(recalc_p_from_f(
      ...
      df1_digits = k_digits,
      df2_digits = 0,
      p_digits = p_model_digits,
      rounding = rounding
    ))
  } else if (has(r2) && has(n) && has(k)) {
    add(recalc_p_model_from_r2(
      r2,
      n,
      k,
      p_model = p_model,
      ...
      n_digits = n_digits,
      k_digits = k_digits,
      p_model_digits = p_model_digits,
      rounding = rounding
    ))
  }
  if (!is.null(beta) && !is.null(r_y) && length(beta) == length(r_y)) {
    add(recalc_r2_from_betas_corrs(
      beta,
      r_y,
      r2 = r2,
      betas_digits = beta_digits,
  ...
  if (
    !is.null(beta) && length(beta) == 2 && !is.null(r_y) && length(r_y) == 2
  ) {
    add(recalc_r12_from_normal_eqs(
      beta[1],
      beta[2],
      r_y[1],
      r_y[2],
      beta1_digits = beta_digits,
      beta2_digits = beta_digits,
      r_y1_digits = r_y_digits,
      r_y2_digits = r_y_digits,
      rounding = rounding
    ))
  }
  if (!is.null(r_y) && length(r_y) == 2 && has(r_12)) {
    add(recalc_betas_from_corrs_k2(
      r_y[1],
      r_y[2],
      r_12,
      betas = beta,
      r_y1_digits = r_y_digits,
      r_y2_digits = r_y_digits,
      r_12_digits = r_12_digits,
      betas_digits = beta_digits,
      rounding = rounding
    ))
  }
  if (!is.null(r_y)) {
    add(recalc_r2_floor_from_correlations(
      r_y,
      r2 = r2,
      r_y_digits = r_y_digits,
      r2_digits = r2_digits,
...
summarise_audit <- function(audit, verbose = TRUE) {
  if (!"predictor" %in% names(audit)) {
    audit$predictor <- "model"
  }
  ...
  if (nrow(diag) > 0) {
    ...
    for (key in unique(keys)) {
      ...
      diag_groups[[length(diag_groups) + 1L]] <- list(
        ...
        resolved_by = if (n_consistent == 1L) {
          g$check[!is.na(cons) & cons][1]
        } else {
          NA_character_
        },
  ...
  any_resolved <- any(vapply(
    diag_groups,
    function(d) !is.na(d$resolved_by),
    logical(1)
  ))
  ...
  if (n_fail == 0 && n_cmp > 0) {
    L <- c(
      ...
      sprintf(
        "All %d direct check(s) pass; every recalculated interval overlaps the reported value's rounding interval.",
        n_cmp
      )
    )
    ...
    resolved_by <- vapply(
      diag_groups,
      function(d) {
        if (!is.na(d$resolved_by)) {
          sprintf("%s (%s)", d$prefix, d$resolved_by)
        } else {
          ""
        }
      },
    ...
    L <- c(
      ...
      sprintf(
        "%d of %d direct checks fail. The following labeling diagnostic(s) are uniquely consistent and would reconcile the report:",
        n_fail,
        n_cmp
      ),
      paste0("  - ", resolved_by)
    ...
    L <- c(
      ...
      sprintf(
        "%d of %d direct checks fail, and no standard label confusion (beta vs b, R^2 vs adjusted R^2, p tails) resolves the discrepancy.",
        n_fail,
        n_cmp
      )
    )
  ...
  if (nrow(fails) > 0) {
    ...
    for (i in seq_len(nrow(fails))) {
      ...
      gap <- interval_gap(
        r$reported_lower,
        r$reported_upper,
        r$recalculated_lower,
        r$recalculated_upper
      )
      ref_line <- if (!is.na(r$reported)) {
        sprintf(
          "    reported:    %s (rounding interval %s)",
          fmt_num(r$reported),
