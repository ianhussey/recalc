...
base_p_cont <- function(df) {
  ...
  nm <- colnames (a) [substr(colnames (a), 1, 1) == "m" | (substr(colnames (a), 1,1) == "s" & colnames (a) != "study")]
  ...
  max_digits = ifelse(sum(!is.na(a$p_dec.place))>0, max(a$p_dec.place, na.rm = T),3)
  
  #add back in missing
  a <- dplyr::bind_rows(a, missing %>% dplyr::select(study, var, p, p_num) %>% 
                          dplyr::mutate(p_flag = "not done because missing data"))
  #change names
  a <- a %>% 
    dplyr::rename(reported_pvalue = p, variable = var, max_possible_p = p_max, min_possible_p = p_min,
                  pvalue_flag= p_flag) %>% 
...
rng_p_fn <- function (dat, nm, nm_m, nm_s, nm_n, nm.m, nm.s, nm.sf) {
  ...
  b_max <- dplyr::left_join (rng %>% dplyr::select (-all_of(nm_m)), 
                             ...
                             by = c("study", "var")) %>% dplyr::select(- c(p, all_of(nm.dtm), all_of(nm.dts), dpm:sfs)) %>% 
  ...
  rng_data <- dplyr::bind_rows (dplyr::left_join(b_max, b_max1 %>% dplyr::rename (p_min = p.value) %>% 
    ...
    dplyr::select(study, var, stat, all_of (nm_m), all_of (nm_s), all_of (nm_n), dfm, dfs, p_range, p_max, p_min)
...
chk_pvals <- function (dat= a, am, nm, nm_m, nm_s, nm_n, nm.m, nm.s, nm.sf) {
  ...
  a1 <- dat %>% dplyr::left_join (rng_p_data %>% dplyr::filter (stat == "min_p-value") %>% 
                                    dplyr::select (study, var, p_min), by = c("study", "var")) %>% 
    dplyr::left_join (rng_p_data %>% dplyr::filter (stat == "max_p-value") %>% dplyr::select (study, var, p_max),
                      by = c("study", "var"))
  # Check whether p-values are possible
  a1 <- a1%>% dplyr::mutate (p_dec.place = dplyr::coalesce(p_dec.place, 3), 
                             across(p_min:p_max, ~ round(.x, p_dec.place)), 
                             p_num_min = dplyr::case_when (grepl(">", p) ~ suppressWarnings(as.numeric(sub(".*>", "", p))),
                                                           grepl("ns", tolower(p)) ~ 0.05,
                                                           is.na(p_num) ~ NA, .default = p_min),
                             p_num_max = dplyr::case_when (grepl("<", p) ~ suppressWarnings(as.numeric(sub(".*<", "", p))),
                                                           is.na(p_num) ~ NA, .default = p_max)) %>% 
    #now put flag for impossible results
    dplyr::mutate (p_flag = dplyr::case_when(is.na(p_num) & is.na(p_num_min) & is.na(p_num_max) ~ NA, 
...
ui <- fluidPage(
  ...
  titlePanel("Check p-values for baseline continuous variables from a randomised controlled trial"),
  
  sidebarLayout(
    sidebarPanel(
      numericInput("groups", "Number of Groups:", value = 2, min = 1),
      numericInput("rows", "Number of Rows:", value = 10, min = 1),
      radioButtons("nN_pattern", "Choose n and N arrangement:",
                   choices = list("n1, n2, n3 ... m1, m2, m3 ... s1, s2, s3 ..." = "nms",
                                  "m1, m2, .... s1, s2, ... n1, n2" = "msn",
                                  "n1, m1, s1, n2, m2, s2 ..." = "n1m1s1",
                                  "m1, s1, n1, m2, s2, n2 ..." = "m1s1n1"),
                   selected = "msn"),
      actionButton("create_table", "Create Table"), br(), br(),
      actionButton("analyze", "Run Analysis"), br(), br(),
    ...
    mainPanel(
      ...
      conditionalPanel(
        ...
        p(style = "color: #666; margin-top: 10px;",
          "Fill in all cells with your data.", br(),
          "You can copy and paste from Excel or type directly into the cells. You can cut and paste multiple rows at once", br(), 
          "Empty cells are allowed for Variable, and P only, otherwise an error will occur", br(),
          "Put the columns in your selected order and use the button to make sure the order in the table matches this order", br(),
          "")), br(),
...
server <- function(input, output, session) {
  ...
  output$table_created <- reactive({!is.null(values$table_data)})
  ...
  get_column_order <- reactive({
    ...
    if (pattern == "nms") {
      col_names <- c(col_names, paste0("n", 1:n_groups), paste0("m", 1:n_groups), paste0("s", 1:n_groups), "p")
    } else if (pattern == "msn") {
      col_names <- c(col_names, paste0("m", 1:n_groups), paste0("s", 1:n_groups), paste0("n", 1:n_groups), "p")
    } else if (pattern == "n1m1s1") {
      col_names <- c(col_names, as.vector(sapply(1:n_groups, function(i) c(paste0("n", i), paste0("m", i), paste0("s", i)))), "p")
    } else if (pattern == "m1s1n1") {
      col_names <- c(col_names, as.vector(sapply(1:n_groups, function(i) c(paste0("m", i), paste0("s", i), paste0("n", i)))), "p")
    }
    return(col_names)
  })
  ...
  observeEvent(input$create_table, {
    ...
    values$table_data <- data.frame(matrix("", nrow = input$rows, ncol = length(columns)))
    names(values$table_data) <- columns
  })
  
  # Render the editable table
  output$data_table <- rhandsontable::renderRHandsontable({
    if (!is.null(values$table_data)) {rhandsontable::rhandsontable(values$table_data, stretchH = "all")}
  })
  
  # Update data when table is edited
  observe({
    if (!is.null(input$data_table)) {values$table_data <- hot_to_r(input$data_table)}
  })
  ...
  output$download <- downloadHandler(
    ...
    content = function(file) {write.csv(values$analysis_results, file, row.names = FALSE)})
  
  output$download_data <- downloadHandler(
    filename = function() {paste("analysis_data_", Sys.Date(), ".csv", sep = "")},
    content = function(file) {write.csv(as.data.frame(values$table_data), file, row.names = FALSE, na = "")})
