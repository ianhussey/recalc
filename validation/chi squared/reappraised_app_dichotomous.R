...
base_p_cat_format <- function(df) {
  ...
  colnames(a)[startsWith(colnames(a), "N")] <- paste0(
    "t",
  ...
  if ("stat" %in% colnames(a)) {
    a <- a %>%
      ...
      tidyr::fill(stat, .direction = "updown")
  }

  if ("p" %in% colnames(a)) {
    a <- a %>%
      dplyr::group_by(study, var) %>%
      tidyr::fill(p, .direction = "updown")
  }

  a <- a %>%
    dplyr::group_by(study, var) %>%
    tidyr::fill(p, starts_with("t"), .direction = "updown") %>%
    dplyr::ungroup()
  ...
  if ("stat" %in% colnames(a)) {
    if (
      ...
      return(
        "Data error- some 'stat' variables don't match for different levels"
      )
    }
  }
  ...
  colnames(a)[colnames(a) %in% names] <- paste0(
    substr(names, 1, 1),
    "_",
    gsub("\\D", "", names)
  )
  ...
  #check groups
  if (!"group" %in% colnames(a)) {
    a$group <- rowSums(!is.na(a[, nm1]))
  }

  #stop if not matching#
  if (length(nm1) != length(nm2) | any(a$group != rowSums(!is.na(a[, nm2])))) {
    return(
      "Data error- some data for groups are missing or group numbers don't match"
    )
  }
  ...
  if (!"calculated_level" %in% colnames(a)) {
    a$calculated_level <- NA_character_
  }
  a <- a %>%
    dplyr::mutate(
      calculated_level = dplyr::case_when(
        !is.na(calculated_level) &
  ...
  a <- a %>%
    dplyr::mutate(
      total_n = rowSums(dplyr::across(all_of(nm1)), na.rm = TRUE)
    ) %>%
    dplyr::arrange(study, var, calculated_level, desc(total_n)) %>%
    dplyr::group_by(study, var) %>%
  ...
  if (any(a$n_all == 1)) {
    l1 <- a %>%
      dplyr::filter(n_all == 1) %>%
      dplyr::mutate(n_all = 2, level = 2) #level 2 calc level =1
    l2 <- l1 %>% dplyr::mutate(level = 1, calculated_level = "yes")

    for (i in seq_along(nm1)) {
      l2[[nm1[i]]] <- l2[[nm2[i]]] - l2[[nm1[i]]]
    } #subtract l2 from l1
  ...
  if (
    a %>%
      dplyr::group_by(study, var) %>%
      dplyr::summarise(
        n = dplyr::n_distinct(calculated_level, na.rm = TRUE),
        .groups = "drop"
      ) %>%
      dplyr::filter(n > 1) %>%
      nrow() >
  ...
  chk <- a %>%
    dplyr::group_by(study, var) %>%
    dplyr::summarise(
      across(all_of(nm1), ~ sum(., na.rm = TRUE)),
      .groups = "drop"
    )
  ...
  a <- a %>%
    ...
    dplyr::mutate(
      ...
      dplyr::across(
        all_of(nm2),
        ~ get(paste0(sub("t", "n", dplyr::cur_column()), "_correct"))
      )
    ) %>%
  ...
  colnames(a)[startsWith(colnames(a), "n")] <- paste0(
    "n",
    gsub("\\D", "", colnames(a)[startsWith(colnames(a), "n")])
  )
  colnames(a)[startsWith(colnames(a), "t")] <- paste0(
    "N",
    gsub("\\D", "", colnames(a)[startsWith(colnames(a), "t")])
  )
  ...
  if ("p" %in% colnames(a)) {
    a <- a %>%
      dplyr::group_by(study, var) %>%
      dplyr::mutate(p = dplyr::first(p[!is.na(p)])) %>%
      dplyr::ungroup()
  }

  #add in stat
  if (stat == 0) {
    a <- dplyr::left_join(a, stat_df, by = c("study", "var"))
  }
...
base_p_cat_calcs <- function(a) {
  ...
  b <- dplyr::left_join(
    b,
    a %>%
      dplyr::select(study, var, levels_no, group) %>%
      dplyr::distinct(study, var, .keep_all = TRUE),
    by = c("study", "var")
  ) %>%
  ...
  chi <- function(x, correct = FALSE) {
    if (nrow(x) == 2L && ncol(x) == 2L) {
      ...
      warning_msg <- if (
        length(exp) == 4L &&
          any(exp < 5) ||
          length(exp) > 4L && (any(exp < 1) || sum(exp < 5) > 0.2 * length(exp))
      ) {
        "Expected cells warning"
      } else {
        ""
      }
  ...
  b <- dplyr::left_join(
    b,
    a %>%
      dplyr::select(study, var, p, stat, stat1) %>%
      dplyr::distinct(study, var, .keep_all = TRUE),
    by = c("study", "var")
  )
  ...
  b <- b %>%
    dplyr::mutate(
      p_n = suppressWarnings(as.numeric(p)),
      p_g = dplyr::case_when(
        grepl("ns", p, ignore.case = TRUE) ~ 0.05,
        grepl(">", p) ~ suppressWarnings(as.numeric(sub('.*>\\s*', '', p))),
        .default = NA
      ),
      p_l = dplyr::case_when(
        grepl("<", p) ~ suppressWarnings(as.numeric(sub('.*<\\s*', '', p))),
      ...
      p = ifelse(is.na(p_n), tolower(p), as.character(p_n))
    )
  ...
  b$p_d <- ifelse(
    !is.na(b$p_n),
    ifelse(
      abs(b$p_n - round(b$p_n)) > .Machine$double.eps^0.5,
      nchar(sub('^\\d+\\.', '', sub('0+$', '', as.character(abs(b$p_n))))),
      0
    ),
    NA
  )
  ...
  b[, p_cols] <- lapply(b[, p_cols], function(x) {
    ifelse(
      !is.na(b$p_n),
      round(x, b$p_d),
      round(x, pmax(b$p_d, 3, na.rm = TRUE))
    )
  })
  ...
  for (i in seq_len(nrow(b))) {
    if (is.na(b$p_g[i]) && is.na(b$p_l[i])) {
      next
    } # no thresholds so skip
    ...
    if (is.na(b$stat[i])) {
      if (!is.na(b$p_g[i])) {
        # Determine which threshold to use (prefer p_g if both exist)
        thrsh <- b$p_g[i]
        comp <- ">"
      } else {
        thrsh <- b$p_l[i]
      ...
      if (b$stat[i] == "midp") {
        if (!is.na(b$p_g[i])) {
          b$m_t[i] <- ifelse(
            b$p_match1[i] > b$p_g[i] & b$p_match2[i] > b$p_g[i],
            "Match",
            "No match"
          )
        } else if (!is.na(b$p_l[i])) {
          b$m_t[i] <- ifelse(
            b$p_match1[i] < b$p_l[i] & b$p_match2[i] < b$p_l[i],
            "Match",
            "No match"
          )
        }
  ...
  b$m_t_comment <- ifelse(
    !is.na(b$m_t_comment),
    paste0(
      toupper(substr(b$m_t_comment, 1, 1)),
      substr(b$m_t_comment, 2, nchar(b$m_t_comment))
    ),
    NA
  )
  ...
  for (i in seq_len(nrow(b))) {
    if (is.na(b$p_n[i])) {
      next
    } # Skip if no reported numeric p-value
    ...
    if (any(matches, na.rm = TRUE)) {
      # Found at least one exact match
    ...
    if (!is.na(b$stat[i])) {
      ...
      if (b$stat[i] == "midp") {
        b$m_rep[i] <- ifelse(
          abs(b$p_n[i] - b$p_midp_exact[i]) < 1e-9,
          "Midp_exact",
          ifelse(abs(b$p_n[i] - b$p_midp_classic[i]) < 1e-9, "Midp_classic", NA)
        )
      } else {
        b$m_rep[i] <- ifelse(
          abs(b$p_n[i] - b[i, nm_p[b$stat[i]]]) < 1e-9,
          b$stat[i],
          NA
        )
      }
  ...
  b$m_rep <- ifelse(
    !is.na(b$m_rep),
    paste0(toupper(substr(b$m_rep, 1, 1)), substr(b$m_rep, 2, nchar(b$m_rep))),
    NA
  )
  b$m_any <- ifelse(
    !is.na(b$m_any),
    paste0(toupper(substr(b$m_any, 1, 1)), substr(b$m_any, 2, nchar(b$m_any))),
    NA
  )
  ...
  b <- dplyr::left_join(
    b,
    a %>%
      dplyr::filter(calculated_level == "yes") %>%
      dplyr::select(study, var, calculated_level) %>%
      dplyr::distinct(),
    by = c("study", "var")
  )
  ...
  calc_p <- function(mat, method) {
    ...
    if (method == "chisq") {
      ...
      p <- fisher_fast(mat)
    } else if (method == "chisqc") {
      p <- chi(mat, correct = TRUE)$p
    } else if (method == "lr") {
      p <- lr(mat)
    } else if (method == "mh") {
      p <- cmh(mat)
      ...
      pf <- fisher_fast(mat)
      p1 <- midp_exact(mat)
      p2 <- midp_classic(mat, pf)
    } else {
      return("Incorrect statistic- see description for valid tests")
    }
  ...
  get_best_p <- function(result, target_p, digits) {
    ...
    if (length(p_values) == 0) {
      return(list(p = NA, diff = Inf, matched = FALSE))
    }
  ...
  test_adjustment <- function(
    ...
    list(
      ...
      p = test_best$p,
      diff = test_best$diff,
      matched = test_best$matched,
      change = change_description,
      improved = test_best$diff < current_best_diff
    )
  }
  ...
  for (i in 1:nrow(b)) {
    if (
      b$m_rep[i] %in%
        ...
        !is.na(b$stat[i]) &
        !is.na(b$p_n[i]) &
        b$calculated_level[i] %in% "yes" &
        ((b$levels_no[i] * b$group[i] > 4 &
          startsWith(tolower(b$stat[i]), "chisq")) |
          b$levels_no[i] * b$group[i] == 4)
    ) {
      counts <- na.omit(as.numeric(b[i, n_cols]))
      x <- matrix(counts, byrow = TRUE, nrow = b$levels_no[i])
      ...
      for (delta in 1:min(x[1, ])) {
        ...
        if (delta > 1 && delta_best_diff >= current_best$diff) {
          break
        }
      }

      if (best_change == "none") {
        b$change_made[i] <- "No match with missing data"
      }

      if (best_change != "none") {
        b$data_missing_match[i] <- paste(
          as.vector(t(best_matrix)),
          collapse = ", "
        )
  ...
  b <- b %>%
    dplyr::mutate(across(
      c(p_chi:p_midp_classic, p_missing_match),
      ~ sprintf(formats, .x)
    ))
  ...
  b <- b %>%
    ...
    dplyr::left_join(
      a %>% dplyr::select(study, var, N_flag) %>% dplyr::distinct(),
      by = c("study", "var")
    ) %>%
    dplyr::rename(
      reported_pvalue = p,
      ...
      any_match = m_any,
      match_reported_p = m_rep,
      threshold_match = m_t,
      threshold_test = m_t_comment
    ) %>%
...
ui <- fluidPage(
  titlePanel(
    "Check p-values for baseline categorical variables from a randomised controlled trial"
  ),
    ...
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        tags$div(
          ...
          tags$div(
            style = "margin-left: 20px; margin-bottom: 10px; color: #555;",
            tags$div("✓ Study"),
            tags$div("✓ Variable"),
            tags$div("✓ n"),
            tags$div("✓ N"),
        ...
        conditionalPanel(
          # Column arrangement for n/N only
          condition = "input.configure > 0",
          hr(),
        ...
        h4("Actions"),
        actionButton("create_table", "Create Table", width = "100%"),
        br(),
        br(),
        actionButton("analyze", "Run Analysis", width = "100%"),
        br(),
        br(),
        ...
        div(
          ...
          a(
            ...
            href = "mailto:m.bolland@auckland.ac.nz"
          ),
          br(),
    ...
    mainPanel(
      ...
      conditionalPanel(
        ...
        p(
          ...
          "• Fill in all cells with your data. Delete or paste over the examples.",
          br(),
          "• You can copy and paste from Excel or type directly into the cells.",
          br(),
          "• You can copy and paste multiple rows at once.",
          br(),
          "• Empty cells are allowed for levels, stat, and P only, otherwise an error will occur",
          br(),
          "• Choose the order of the columns using the button.",
          br(),
          "• n1 is the number in group1 for the level (eg 10 males in example 1), N1 is the total number in group 1 (eg 20) or the sum of n1 for all levels if there are missing data",
          br(),
          "• Variable should be the same for each variable, level is optional and can describe the level eg variable gender, level male in example 1. If there are 2 levels (Male, Female), you can enter only 1 level as in example 2 and the second level is calculated automatically",
          br(),
          "• Permitted stats are 'chisq' for standard chisquare, 'chisqc' for chisq + continuity correction, 'fisher' for fisher's exact, 'lr' for likelihood ratio, 'mh' for CMH test, 'midp' for midp tests",
          br(),
          "• Permitted p-values are 'ns' for not-significant, '>x' for > any value, '<x' for < any value, or the reported number",
          br(),
        ...
        conditionalPanel(
          ...
          p(
            ...
            "Notes:",
            br(),
            "• reported_pvalue: p-value from above table",
            br(),
            "• any_match: the matching statistical test if it matches any calculated p-value",
            br(),
            "• match_reported_p: if reported p-value matches the calculated p-value using the reported statistical test",
            br(),
            "• threshold match: if the p-value is a threshold, it indicates whether it matches the calculated p-value for the reported test or any p-value when no reported statistical test",
            br(),
            "• threshold test: statistical test used to compare p-value with threshold",
            br(),
            "• p_chi to p_midp: calculated p-values using various tests, midp_exact (eg epitools in R), midp_classic (eg midp in SAS) see code for calculations",
            br(),
            "• chi_warning: if expected cell <5 for 2*2 or the relevant rule for >2*2 tables",
            br(),
            "• p_missing_match: matching p_value from progressively removing a maximum of 1/group for each group then 2/group etc; data_missing_match: first matching set of n/N giving matching p_value; change_made: changes made to N to get matching set",
            br(),
...
server <- function(input, output, session) {
  ...
  get_column_order <- reactive({
    ...
    if ("level" %in% selected_cols) {
      col_list <- c(col_list, "level")
    }
    has_N <- "N" %in% selected_cols # Add n and N columns based on pattern
    if (has_N) {
      if (input$nN_pattern == "alternating") {
        # Both n and N selected - use pattern
        for (i in 1:n_groups) {
          col_list <- c(col_list, paste0("n", i), paste0("N", i))
        } # n1, N1, n2, N2, ...
      } else {
        # n1, n2, ..., N1, N2, ...
        for (i in 1:n_groups) {
          col_list <- c(col_list, paste0("n", i))
        }
        for (i in 1:n_groups) {
          col_list <- c(col_list, paste0("N", i))
        }
      ...
      # Only n columns (always required)
      for (i in 1:n_groups) {
        col_list <- c(col_list, paste0("n", i))
      }
    }
    ...
    if ("stat" %in% selected_cols) {
      col_list <- c(col_list, "stat")
    }
    if ("calculated_level" %in% selected_cols) {
      col_list <- c(col_list, "calculated_level")
    }
    return(col_list)
  })
  ...
  observe({
    ...
    colnames(df) <- columns
    df[1, c("Study", "Variable", "n1", "n2", "p", "N1", "N2")] <- c(
      "Example1",
      "Gender",
      10,
      15,
    ...
    if (n_rows > 1) {
      df[2, c("Study", "Variable", "n1", "n2", "p", "N1", "N2")] <- c(
        "Example1",
        "Gender",
        10,
        5,
    ...
    if (n_rows > 2) {
      df[3, c("Study", "Variable", "n1", "n2", "N1", "N2")] <- c(
        "Example2",
        "Gender",
        10,
        5,
    ...
    if ("level" %in% columns) {
      df$level[1:n_rows] <- c("M", "F")[n_rows]
    }
    if ("stat" %in% columns) {
      df$stat[1:n_rows] <- "fisher"
    }
      ...
      tryCatch(
        {
          ...
          if (is.character(a) && length(a) == 1) {
            stop(a)
          } # Throw error with the message from the function
  ...
  output$download <- downloadHandler(
    ...
    content = function(file) {
      write.csv(values$analysis_results, file, row.names = FALSE, na = "")
    }
  )
  ...
  output$download_data <- downloadHandler(
    ...
    content = function(file) {
      write.csv(values$analysis_data, file, row.names = FALSE, na = "")
    }
  )
