...
.webapp_chi <- function(x, correct = FALSE) {
  if (nrow(x) == 2L && ncol(x) == 2L) {            # fast 2x2
    a <- x[1, 1]; b <- x[1, 2]; c <- x[2, 1]; d <- x[2, 2]; n <- a + b + c + d
    if (correct) {
      chi_stat <- n * (max(abs(a * d - b * c) - n / 2, 0))^2 /
        ((a + b) * (c + d) * (a + c) * (b + d))
    } else {
      chi_stat <- n * (a * d - b * c)^2 /
        ((a + b) * (c + d) * (a + c) * (b + d))
    }
    ...
    if (correct && nr == 2L && nc == 2L) {
      ...
      diff <- x - exp; chi_stat <- sum(diff * diff / exp)
    }
    df <- (nr - 1L) * (nc - 1L)
...
.webapp_lr <- function(obs) {
  row <- rowSums(obs); col <- colSums(obs); n <- sum(obs)
  exp <- outer(row, col) / n
...
.webapp_midp_classic <- function(x, p_fisher) {
  a <- x[1, 1]; b <- x[1, 2]; c <- x[2, 1]; d <- x[2, 2]; n <- sum(x)
...
reappraised_webapp_pvalues <- function(tab) {
  if (!(nrow(tab) == 2L && ncol(tab) == 2L)) {
    stop("reappraised_webapp_pvalues() is defined for 2x2 tables (the app's mid-p path).")
  }
...
reappraised_webapp_methods <- c("pearson", "yates", "fisher", "likelihood_ratio",
                                "n1_chisq", "fisher_midp", "fisher_midp_sas")
