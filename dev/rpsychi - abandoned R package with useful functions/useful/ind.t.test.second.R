...
ind.t.test.second <-
  function(
    ...
    ##(a) sample statistics
    samp.stat <- round(
      c(m1 = m1, sd1 = sd1, n1 = n1, m2 = m2, sd2 = sd2, n2 = n2),
      digits
    )
    ...
    raw.difference <- round(
      c(mean.diff = psi, lower = psi.lower, upper = psi.upper, std = psi.std),
      digits
    )
    ...
    standardized.difference <- round(
      c(es = g, lower = g.lower, upper = g.upper, std = g.std),
      digits
    )
    ...
    criterion.power <- round(
      power.t(sig.level = sig.level, delta = c.delta, n1 = n1, n2 = n2),
      digits
    )
