...
ind.prop.second <-
  function(x, n, sig.level = .05, digits = 3, ref.ind = 1) {
    ...
    samp.stat <- round(
      c(p1 = p1, n1 = n1, p2 = p2, n2 = n2, cohen.h = cohen.h),
      digits
    )
    ...
    risk.difference <- round(
      c(es = rd, lower = rd.lower, upper = rd.upper, std = rd.sem),
      digits
    )
    ...
    rr.lower <- exp(
      log(rr) + qnorm(sig.level / 2, lower.tail = TRUE) * log.rr.sem
    )
    rr.upper <- exp(
      log(rr) + qnorm(sig.level / 2, lower.tail = FALSE) * log.rr.sem
    )
    risk.ratio <- round(
      c(es = rr, lower = rr.lower, upper = rr.upper, log.rr.sem = log.rr.sem),
      digits
    )
    ...
    or.lower <- exp(
      log(or) + qnorm(sig.level / 2, lower.tail = TRUE) * log.or.sem
    )
    or.upper <- exp(
      log(or) + qnorm(sig.level / 2, lower.tail = FALSE) * log.or.sem
    )
    odds.ratio <- round(
      c(es = or, lower = or.lower, upper = or.upper, log.or.sem = log.or.sem),
      digits
    )

    #power
    criterion.power <- round(
      c(
        small = power.prop(h = .2, n = n, sig.level = sig.level),
        medium = power.prop(h = .5, n = n, sig.level = sig.level),
        large = power.prop(h = .8, n = n, sig.level = sig.level)
      ),
      digits
    )
