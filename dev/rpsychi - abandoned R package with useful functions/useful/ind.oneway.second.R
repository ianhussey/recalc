...
ind.oneway.second <- function(
  ...
  delta.lower <- try(
    FNONCT(f.value, dfb, dfw, prob = 1 - sig.level / 2),
    silent = TRUE
  )
  delta.upper <- try(
    FNONCT(f.value, dfb, dfw, prob = sig.level / 2),
    silent = TRUE
  )
  ...
  omnibus.es <- round(
    c(etasq = etasq, etasq.lower = etasq.lower, etasq.upper = etasq.upper),
    digits
  )
  ...
  criterion.power <- round(
    power.f(sig.level = sig.level, u = dfb, n = sum(n) / k, delta = c.delta),
    digits
  )
  ...
  output <- list(
    ...
    raw.contrasts = raw.contrasts,
    standardized.contrasts = standardized.contrasts,
