...
ind.twoway.second <-
  function(m, sd, n, unbiased = TRUE, sig.level = .05, digits = 3) {
    ...
    tmp.function <- function(etasq, df1, df2, sig.level = sig.level) {
      ...
      for (i in 1:iter) {
        delta.lower[i] <- try(
          FNONCT(f.value[i], df1[i], df2, prob = 1 - sig.level / 2),
          silent = TRUE
        )
        delta.upper[i] <- try(
          FNONCT(f.value[i], df1[i], df2, prob = sig.level / 2),
          silent = TRUE
        )
      }
      ...
      output <- data.frame(
        partial.etasq = etasq,
        partial.etasq.lower = lower.etasq,
    ...
    omnibus.es <- tmp.function(
      c(etasq.row, etasq.col, etasq.int),
    ...
    ##(c) statistical power
    c.delta <- c(.10, .25, .4)
    criterion.power <- rbind(
      power.f2(sig.level = sig.level, df1 = dfrow, df2 = dfw, delta = c.delta),
