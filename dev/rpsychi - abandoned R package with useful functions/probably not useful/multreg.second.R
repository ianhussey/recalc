multreg.second <- function(
  ...
  ##(a) correlation
  corr.lower <- tanh(
    atanh(corr) + qnorm(sig.level / 2, lower.tail = TRUE) / sqrt(n - 3)
  )
  corr.upper <- tanh(
    atanh(corr) + qnorm(sig.level / 2, lower.tail = FALSE) / sqrt(n - 3)
  )
  ...
  ##output
  output <- list(
    corr.partial.corr = cor.mat,
    corr.confidence = corr.conf,
