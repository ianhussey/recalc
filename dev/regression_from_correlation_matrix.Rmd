...
regression_from_correlation_matrix <- function(model, cor, n){
  ...
  res <- 
    ...
    select(predictor = rhs,
           Std.Beta = est,
           Std.Beta_se = se,
           Std.Beta_ci_lower = ci.lower,
           Std.Beta_ci_upper = ci.upper,
           p = pvalue) |>
...
Original model, described as non-causal:

$prosocial \sim stress + affect + support + flexiblity$

Alternative models, which can nudge authors to consider their implicit causal assumptions:
