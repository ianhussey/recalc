...
multiverse_independent_t_d <- function(
  ...
  if (!is.null(ci_methods)) {
    ...
    if (length(bad) > 0L) {
      stop(
        ...
        paste0(bad, collapse = ", "),
        "\nAllowed: ",
        paste0(allowed_ci_methods, collapse = ", ")
      )
    }
  }
  ...
  if (!is.null(p_methods)) {
    ...
    if (length(bad) > 0L) {
      stop(
        ...
        paste0(bad, collapse = ", "),
        "\nAllowed: ",
        paste0(allowed_p_methods, collapse = ", ")
      )
    }
  }
  ...
  if (!is.null(d_rounding_set)) {
    ...
    if (length(bad) > 0L) {
      stop(
        ...
        paste0(bad, collapse = ", "),
        "\nAllowed: ",
        paste0(allowed_d_rounding, collapse = ", ")
      )
    }
  }
  ...
  nct_ci <- function(t_obs, df, alpha = 0.05, max_ncp = 1000) {
    ...
    fU <- function(delta) {
      stats::pt(t_obs, df = df, ncp = delta) - (1 - alpha / 2)
    }
    ...
    for (adj_tdf in adj_codes) {
      ...
      for (p_method in p_methods) {
        if (p_method %in% c("student_t", "welch_t")) {
          ...
          # student_z, welch_z: treat t_star as z
          t_use <- t_star
          df_use <- Inf
          p_unr <- 2 * (1 - stats::pnorm(abs(t_use)))
        }
