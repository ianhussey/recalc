multiverse_one_or_two_way_between_anova <- function(
  ...
  if (length(bad <- setdiff(F_rounding_set, allowed_F_round))) {
    stop(
      ...
      paste0(bad, collapse = ", "),
      ". Allowed: ",
      paste0(allowed_F_round, collapse = ", ")
    )
  }
  if (length(bad <- setdiff(p_rounding_set, allowed_p_round))) {
    stop(
      "p_rounding_set contains invalid values: ",
      paste0(bad, collapse = ", "),
      ". Allowed: ",
      paste0(allowed_p_round, collapse = ", ")
    )
  }
  if (length(bad <- setdiff(peta2_rounding_set, allowed_eta_round))) {
    stop(
      "peta2_rounding_set contains invalid values: ",
      paste0(bad, collapse = ", "),
      ". Allowed: ",
      paste0(allowed_eta_round, collapse = ", ")
    )
  }
  if (length(bad <- setdiff(input_adj_codes, allowed_adj))) {
    stop(
      "input_adj_codes contains invalid values: ",
      paste0(bad, collapse = ", "),
      ". Allowed: ",
      paste0(allowed_adj, collapse = ", ")
    )
  }
          ...
          for (p_round in p_rounding_set) {
            ...
            for (eta_round in peta2_rounding_set) {
              ...
              results[[idx]] <- data.frame(
                ...
                match_F = if (!is.na(F_est_num)) {
                  isTRUE(all.equal(F_rep, F_est_num))
                } else {
                  NA
                },
                match_df1 = if (!is.na(df1_est_num)) {
                  isTRUE(all.equal(df1, df1_est_num))
                } else {
                  NA
                },
                match_df2 = if (!is.na(df2_est_num)) {
                  isTRUE(all.equal(df2, df2_est_num))
                } else {
                  NA
                },
                match_p = if (!is.na(p_est_num)) {
                  isTRUE(all.equal(p_rep, p_est_num))
                } else {
                  NA
                },
                match_peta2 = if (!is.na(peta2_est_num)) {
                  isTRUE(all.equal(eta_rep, peta2_est_num))
                } else {
                  NA
                },
...
labels <- c(
  "groups 1 (A vs B)",
  "groups 2 (C vs D)",
  "groups 1 x groups 2 interaction"
)
