...
cohens_d <- function(mean1, sd1, n1, mean2, sd2, n2){
  tibble(m = (n1 + n2) - 2,
         sdp = sqrt(((n1 - 1) * sd1^2 + (n2 - 1) * sd2^2)/m),
         effect_size = (m1 - m2)/sdp,
         variance = 1/n1 + 1/n2 + effect_size^2/(2 * (n1 + n2)),
         standard_error = sqrt(variance)) |>
    mutate(ci_lower = effect_size - (1.96 * standard_error),
           ci_upper = effect_size + (1.96 * standard_error)) |>
    select(effect_size = effect_size, standard_error = standard_error, ci_lower = ci_lower, ci_upper = ci_upper)
}

glass_delta_sd1 <- function(mean1, sd1, n1, mean2, n2){
  tibble(m = (n1 + n2) - 2,
         effect_size = (m1 - m2)/sd1,
         variance = 1/n1 + 1/n2 + effect_size^2/(2 * (n1 + n2)),
         standard_error = sqrt(variance)) |>
    mutate(ci_lower = effect_size - (1.96 * standard_error),
           ci_upper = effect_size + (1.96 * standard_error)) |>
...
glass_delta_sd2 <- function(mean1, n1, mean2, sd2, n2){
  tibble(m = (n1 + n2) - 2,
         effect_size = (m1 - m2)/sd2,
         variance = 1/n1 + 1/n2 + effect_size^2/(2 * (n1 + n2)),
         standard_error = sqrt(variance)) |>
    mutate(ci_lower = effect_size - (1.96 * standard_error),
           ci_upper = effect_size + (1.96 * standard_error)) |>
...
cohens_d_from_summary_statistics <- function(mean1, sd1, n1, mean2, sd2, n2) {
  bind_rows(
    cohens_d(n1 = n1,
             mean1 = mean1,
             sd1 = sd1,
    ...
    escalc(measure = "SMD",
      ...
      rename(effect_size = yi,
             variance = vi) |>
      mutate(type = "Hedges' g",
             standard_error = sqrt(variance),
             ci_lower = effect_size - (1.96 * standard_error),
             ci_upper = effect_size + (1.96 * standard_error)) |>
      select(-variance),
    glass_delta_sd1(n1 = n1,
                    mean1 = mean1,
                    sd1 = sd1,
                    n2 = n2,
                    mean2 = mean2) |>
      mutate(type = "Glass's delta (sd1)"),
    glass_delta_sd2(n1 = n1,
                    mean1 = mean1,
                    n2 = n2,
                    mean2 = mean2,
                    sd2 = sd2) |>
      mutate(type = "Glass's delta (sd2)"),
  ) |>
    relocate(type, .before = "effect_size")
} 
  
cohens_d_from_summary_statistics(mean1 = 13.1, sd1 = 3.4, n1 = 28, mean2 = 12.1, sd2 = 3.6, n2 = 29) |>
  mutate_if(is.numeric, round_half_up, digits = 2) |>
  knitr::kable() |>
  kableExtra::kable_classic(full_width = FALSE)
