...
.reported_rows <- function(df) {
  df[
    df$input_adj_stats == "m1:reported|m2:reported|sd1:reported|sd2:reported",
    ,
    drop = FALSE
  ]
}
...
test_that("one-sided alternatives match the directed tail", {
  ...
  expect_equal(
    rep$p_unrounded[rep$alternative == "two.sided"],
    2 * pt(-abs(t_s), df = df),
    tolerance = 1e-12
  )
  expect_equal(
    rep$p_unrounded[rep$alternative == "less"],
    pt(t_s, df = df, lower.tail = TRUE),
    tolerance = 1e-12
  )
  expect_equal(
    rep$p_unrounded[rep$alternative == "greater"],
    pt(t_s, df = df, lower.tail = FALSE),
    tolerance = 1e-12
  )
...
test_that("d_formulas produce the documented denominators", {
  ...
  expect_equal(
    unique(rep$d_unrounded[rep$d_formula == "pooled_df"]),
    (s$m1 - s$m2) / sp_df,
    tolerance = 1e-12
  )
  expect_equal(
    unique(rep$d_unrounded[rep$d_formula == "pooled_n"]),
    (s$m1 - s$m2) / sp_n,
    tolerance = 1e-12
  )
  expect_equal(
    unique(rep$d_unrounded[rep$d_formula == "simple_avg"]),
    (s$m1 - s$m2) / sp_avg,
    tolerance = 1e-12
  )
})
...
test_that("Hedges' correction variants match expected J values", {
  ...
  expect_equal(
    unique(rep$d_unrounded[rep$hedges_correction == "none"]),
    d_raw,
    tolerance = 1e-12
  )
  expect_equal(
    unique(rep$d_unrounded[rep$hedges_correction == "approx"]),
    J_approx * d_raw,
    tolerance = 1e-12
  )
  expect_equal(
    unique(rep$d_unrounded[rep$hedges_correction == "exact"]),
    J_exact * d_raw,
    tolerance = 1e-12
  )
...
test_that("reproduced_p_summary exposes rounded bounds and uses them in p_inbounds", {
  ...
  expect_true(all(
    c("min_p", "max_p", "min_p_rounded", "max_p_rounded", "p_inbounds") %in%
      names(res$reproduced)
  ))
  expect_equal(
    res$reproduced$min_p_rounded,
    roundwork::round_up(res$reproduced$min_p, 2),
    tolerance = 1e-12
  )
  expect_equal(
    res$reproduced$max_p_rounded,
...
test_that("a reported d equal to the rounded max bound is in bounds", {
  ...
  for (d_rep in c(min_rd, max_rd)) {
    ...
    expect_true(
      isTRUE(res$reproduced$d_inbounds),
