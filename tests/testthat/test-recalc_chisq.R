...
test_that("n1_chisq is (N-1)/N * Pearson", {
  ...
  expect_equal(
    recalc_chisq_p(counts = tab, p_methods = "n1_chisq")$p_results$p_unrounded[1],
...
test_that("fisher_central is the equal-tailed (2*min one-sided) convention", {
  ...
  a <- tab[1, 1]; r1 <- sum(tab[1, ]); r2 <- sum(tab[2, ]); c1 <- sum(tab[, 1])
  central <- min(1, 2 * min(phyper(a, r1, r2, c1),
                            phyper(a - 1, r1, r2, c1, lower.tail = FALSE)))
  expect_equal(
    recalc_chisq_p(counts = tab, p_methods = "fisher_central")$p_results$p_unrounded[1],
    central
  )
...
test_that("the default method set spans all nine methods, dropping 2x2-only for r x c", {
  ...
  expect_setequal(
    ...
    c("pearson", "yates", "likelihood_ratio", "n1_chisq", "fisher",
      "fisher_central", "fisher_midp", "fisher_midp_sas", "wald_z")
  )
  r3 <- suppressMessages(
    recalc_chisq_p(counts = matrix(c(10, 20, 15, 12, 8, 18), 3, byrow = TRUE)))
  # 2x2-only methods gone; scaled-Pearson n1_chisq retained
  expect_true(all(c("pearson", "likelihood_ratio", "fisher", "n1_chisq") %in%
                    r3$p_results$method))
  expect_false(any(c("yates", "fisher_central", "fisher_midp",
                     "fisher_midp_sas", "wald_z") %in% r3$p_results$method))
})

test_that("new methods compose into the gap-aware union reproduced summary", {
  r <- recalc_chisq_p(counts = matrix(c(3, 12, 9, 6), 2, byrow = TRUE),
                      p = 0.05, p_digits = 3)
  expect_true(all(c("p_inbounds", "p_inbounds_hull", "covered_width",
                    "hull_width", "covered_fraction") %in% names(r$reproduced)))
